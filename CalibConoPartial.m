...
for i = 1:length(reg)
  ...
  [~, indexes{i}] = ...
    max(imgGray(:, reg{i}(1):reg{i}(2)));
  % Calcular los subpeaks
   delta = 7;
   indexSub{i} = maxSubPeak(imgGray, reg{i}(1):reg{i}(2), indexes{i}, delta);
end
...
for i = 1:length(reg)
  Recta{i} = fitLine(reg{i}(1, 1):reg{i}(1, 2), indexSub{i});
end
...
figure
plot(PtsTourn(1, :), PtsTourn(2, :), '.'); axis equal
...
fclose(file);
%%
function H_matrix = get_homography(Pi, Pw)
a1 = zeros(18);
for i = 1:9
x = PtsCono(1,i);
y = PtsCono(2,i);
...
%%
function Recta = fitLine(intervalX, y)
...
function maxPeakSub = maxSubPeak(imgGray, intervalLines, maxPeaks, delta)
imgt = imgGray(:, rang);
% figure,imshow(imgt);
% figure;
for j = 1:length(rang)
